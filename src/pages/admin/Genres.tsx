...
export default function AdminGenres() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-bold tracking-tight mb-6">Genres</h1>
      ...
      <div className="glass-card overflow-hidden max-w-xl">
        {data?.map((genre) => (
          <div key={genre.id} className="flex items-center justify-between p-4 border-b border-white/[0.06] last:border-none">
            {editing?.id === genre.id ? (
              <div className="flex items-center gap-2 flex-1">
                ...
                <button onClick={() => updateMutation.mutate({ id: genre.id, name: editing.name })} className="p-2 text-green-400 hover:text-green-300 rounded-full hover:bg-white/10"><FiSave className="w-4 h-4" /></button>
                <button onClick={() => setEditing(null)} className="p-2 text-white/40 hover:text-white rounded-full hover:bg-white/10"><FiX className="w-4 h-4" /></button>
              </div>
            ) : (
              <>
                <span className="font-medium">{genre.name}</span>
                <div className="flex items-center gap-2">
                  <button onClick={() => setEditing({ id: genre.id, name: genre.name })} className="p-2 text-white/40 hover:text-white rounded-full hover:bg-white/10"><FiEdit2 className="w-4 h-4" /></button>
                  <button onClick={() => { if (confirm('Delete this genre?')) deleteMutation.mutate(genre.id) }} className="p-2 text-white/40 hover:text-red-400 rounded-full hover:bg-white/10"><FiTrash2 className="w-4 h-4" /></button>
                </div>
              </>
            )}
          </div>
