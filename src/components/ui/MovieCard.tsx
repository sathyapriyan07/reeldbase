...
export default function MovieCard({ movie, rating }: Props) {
  return (
    <Link to={`/movie/${movie.slug}`} className="card group block">
      <div className="relative aspect-[2/3] overflow-hidden">
        {movie.poster_url ? (
          ...
          <div className="w-full h-full bg-apple-900 flex items-center justify-center">
            <span className="text-4xl font-bold text-white/20">{movie.title[0]}</span>
          </div>
        )}
      ...
      <div className="p-3.5">
        ...
        {movie.release_date && (
          <p className="text-xs text-white/40 mt-1">{formatDate(movie.release_date).split(',')[0]}</p>
        )}
      </div>
    </Link>
  )
...
export function SeriesCard({ series }: { series: any }) {
  return (
    <Link to={`/series/${series.slug}`} className="card group block">
      <div className="relative aspect-[2/3] overflow-hidden">
        {series.poster_url ? (
          <img
          ...
          <div className="w-full h-full bg-apple-900 flex items-center justify-center">
            <span className="text-4xl font-bold text-white/20">{series.title[0]}</span>
          </div>
        )}
        <div className="absolute top-3 left-3">
...
export function PersonCard({ person }: { person: any }) {
  return (
    <Link to={`/person/${person.slug}`} className="card group block text-center">
      <div className="relative aspect-[3/4] overflow-hidden">
        {person.profile_url ? (
          ...
          <div className="w-full h-full bg-apple-900 flex items-center justify-center">
            <span className="text-3xl font-bold text-white/20">{person.name[0]}</span>
          </div>
        )}
      </div>
      <div className="p-3.5">
        ...
        {person.role && (
          <p className="text-xs text-white/40 mt-1 capitalize">{person.role.replace('_', ' ')}</p>
        )}
      </div>
    </Link>
  )
